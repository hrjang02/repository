...
for i in months:
    df = pd.read_csv(f'/data02/dongnam/met/ASOS/raw/2015{i}.csv')
    tot_df = pd.concat([tot_df,df],ignore_index=True)
#%%    
df_sorted = tot_df.sort_values(by='stnId', ascending=True)

#%%
ori_stn_info = pd.read_csv(f'/home/hrjang2/0_code/code/ASOS_info.csv',encoding='cp949')
stn_info = pd.read_csv(f'/home/hrjang2/0_code/code/ASOS_info.csv',encoding='cp949')
...
stn_info = stn_info.drop_duplicates(subset=['지점'])
#%%
